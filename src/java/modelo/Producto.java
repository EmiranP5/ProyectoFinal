...
public class Producto {
    ...
    public HashMap mostrar_producto()
    {
        ...
        try
        {
            String codigo_sql="SELECT id_producto as id, producto FROM productos WHERE estado = 1;";
            cn= new Conexion();
            cn.abrir_conexion();
   ...
   public int agregar()
    {
        ...
        try
        {
            ...
            PreparedStatement parametro;
            String codigo_sql="INSERT INTO productos (producto, descripcion, imagen, fecha_ingreso, id_marca, existencia, precio_costo, precio_venta) VALUES (?,?,?,?,?,?,?,?);";
            cn = new Conexion();
            cn.abrir_conexion();
            parametro=(PreparedStatement) cn.conexionBD.prepareStatement(codigo_sql);
   ...
   public DefaultTableModel leer() {
        ...
        try {
            cn = new Conexion();
            String query = "SELECT p.id_producto AS id, p.producto, p.descripcion, p.imagen, p.precio_costo, p.precio_venta, p.existencia, p.fecha_ingreso, m.marca, p.id_marca FROM productos AS p INNER JOIN marcas AS m ON p.id_marca = m.id_marca ORDER BY p.id_producto;";
            cn.abrir_conexion();
   ...
   public DefaultTableModel leer2() {
    ...
    public HashMap<String, String> drop_productos() {
        ...
        try {
            cn = new Conexion();
            String query = "SELECT id_producto as id, producto FROM productos WHERE estado = 1;"; // Solo muestra proveedores activos
            cn.abrir_conexion();
